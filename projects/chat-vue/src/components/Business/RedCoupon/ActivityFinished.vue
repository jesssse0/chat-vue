<template>
  <section class="advanced-success-container">
    <img class="icon-finished" src="../../../assets/images/redcoupon/icon-finished.png" alt="" />
    ...
    <template v-if="!isHideUseBtn">
      <div class="btn-wrapper" v-if="!isPC">
        <div class="btn btn-use" @click="onMoreBenefits">{{ afterReceiveBtn }}</div>
      </div>
    </template>
  </section>
</template>
...
export default {
  ...
  computed: {
    ...
    afterReceiveBtn() {
      const { afterReceiveBtn = '查看更多车主福利' } = this.activityInfo;
      return afterReceiveBtn;
    },
    isPC() {
